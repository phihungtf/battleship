...
let selectedShipNameWithIndex, draggedShip, draggedShipLength;
...
function startMultiPlayer() {
	...
	socket.on('player-number', (num) => {
		if (num === -1) {
			infoDisplay.innerText = 'Sorry, the server is full';
		} else {
	...
	socket.on('timeout', () => {
		infoDisplay.innerText = 'You have reached the 10 minute limit';
	});
	...
	startButton.addEventListener('click', () => {
		...
		} else infoDisplay.innerText = 'Please place all ships';
	});
...
function dragDrop() {
	// console.log(this);
	let shipNameWithLastId = draggedShip.querySelector('div:last-child').id;
...
function playGameMulti(socket) {
	...
	if (enemyReady) {
		if (currentPlayer === 'user') {
			turnDisplay.innerText = 'Your Go';
		}
		if (currentPlayer === 'enemy') {
			turnDisplay.innerText = "Enemy's Go";
		}
	}
}
...
function playGameSingle() {
	...
	if (currentPlayer === 'enemy') {
		turnDisplay.innerText = 'Computers Go';
...
function checkForWin() {
	...
	if (destroyerCount === 2) {
		infoDisplay.innerText = `Your sunk the ${enemy}'s destroyer`;
		destroyerCount = 10;
	}
	if (submarineCount === 3) {
		infoDisplay.innerText = `Your sunk the ${enemy}'s submarine`;
		submarineCount = 10;
	}
	if (cruiserCount === 3) {
		infoDisplay.innerText = `Your sunk the ${enemy}'s cruiser`;
		cruiserCount = 10;
	}
	if (battleshipCount === 4) {
		infoDisplay.innerText = `Your sunk the ${enemy}'s battleship`;
		battleshipCount = 10;
	}
	if (carrierCount === 5) {
		infoDisplay.innerText = `Your sunk the ${enemy}'s carrier`;
		carrierCount = 10;
	}
	if (cpuDestroyerCount === 2) {
		infoDisplay.innerText = `${enemy} sunk your destroyer`;
		cpuDestroyerCount = 10;
	}
	if (cpuSubmarineCount === 3) {
		infoDisplay.innerText = `${enemy} sunk your submarine`;
		cpuSubmarineCount = 10;
	}
	if (cpuCruiserCount === 3) {
		infoDisplay.innerText = `${enemy} sunk your cruiser`;
		cpuCruiserCount = 10;
	}
	if (cpuBattleshipCount === 4) {
		infoDisplay.innerText = `${enemy} sunk your battleship`;
		cpuBattleshipCount = 10;
	}
	if (cpuCarrierCount === 5) {
		infoDisplay.innerText = `${enemy} sunk your carrier`;
		cpuCarrierCount = 10;
	}
	...
	if (
		...
		infoDisplay.innerText = `${enemy.toUpperCase()} WINS`;
		gameOver();
	}
}
